    ...
    let
      ...
      existingIp =
        if existingNetProtoFam == MultiCodec.codec("ip6"):
          IpAddress(family: IPv6, address_v6: array[16, byte].initCopyFrom(existingNetProtoAddr))
        else:
          IpAddress(family: IPv4, address_v4: array[4, byte].initCopyFrom(existingNetProtoAddr))
